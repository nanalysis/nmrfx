...
/**
 * <p>
 * This class contains Java translations of the MINPACK nonlinear least squares routines. As of November 2000, it does
 * not yet contain the MINPACK solvers of systems of nonlinear equations. They should be added in the Spring of 2001
 * .<p>
 ...
 *
 * <p>
 * <b>IMPORTANT:</b> The "_f77" suffixes indicate that these routines use FORTRAN style indexing. For example, you will
 * see
 * <pre>
 *   for (i = 1; i <= n; i++)
 * </pre> rather than
 * <pre>
 *   for (i = 0; i < n; i++)
 * </pre> To use the "_f77" routines you will have to declare your vectors and matrices to be one element larger (e.g.,
 * v[101] rather than v[100], and a[101][101] rather than a[100][100]), and you will have to fill elements 1 through n
 * rather than elements 0 through n - 1. Versions of these programs that use C/Java style indexing might eventually be
 * available. They would end with the suffix "_j".
 *
 * <p>
 * This class was translated by a statistician from FORTRAN versions of lmder and lmdif. It is NOT an official
 * translation. It wastes memory by failing to use the first elements of vectors. When public domain Java optimization
 * routines become available from the people who produced MINPACK, then <b>THE CODE PRODUCED BY THE NUMERICAL ANALYSTS
 * SHOULD BE USED</b>.
 *
 * <p>
...
public class Minpack_f77 extends Object {
    ...
    /**
     * <p>
     * The lmder1_f77 method minimizes the sum of the squares of m nonlinear functions in n variables by a modification
     * of the Levenberg-Marquardt algorithm. This is done by using the more general least-squares solver lmder_f77. The
     * user must provide a method which calculates the functions and the Jacobian.
     * <p>
     * Translated by Steve Verrill on November 17, 2000 from the FORTRAN MINPACK source produced by Garbow, Hillstrom,
     * and More.<p>
     *
     * @param nlls A class that implements the Lmder_fcn interface (see the definition in Lmder_fcn.java). See
     *             LmderTest_f77.java for an example of such a class. The class must define a method, fcn, that must have the form
     *             <p>
     *             public static void fcn(int m, int n, double x[], double fvec[], double fjac[][], int iflag[])
     *             <p>
     *             If iflag[1] equals 1, fcn calculates the values of the m functions [residuals] at x and returns this vector in
     *             fvec. If iflag[1] equals 2, fcn calculates the Jacobian at x and returns this matrix in fjac (and does not alter
     *             fvec).
     *             <p>
     *             The value of iflag[1] should not be changed by fcn unless the user wants to terminate execution of lmder_f77. In
     *             this case set iflag[1] to a negative integer.
     ...
     * @param x    On input, it contains the initial estimate of the solution vector [the least squares parameters]. On
     ...
     *             relative error in the sum of squares is at most tol or that the relative error between x and the solution is at
     *             most tol.
     * @param info An integer output variable. If the user has terminated execution, info is set to the (negative) value
     *             of iflag[1]. See description of fcn. Otherwise, info is set as follows.
     *             <p>
     ...
     *             <p>
     *             info = 5 number of calls to fcn with iflag[1] = 1 has reached 100*(n+1).
     *             <p>
     ...
     * @param ipvt An integer output array of length n. ipvt defines a permutation matrix P such that jac*P = QR, where
     *             jac is the final calculated Jacobian, Q is orthogonal (not stored), and R is upper triangular with diagonal
     *             elements of nonincreasing magnitude. Column j of P is column ipvt[j] of the identity matrix.
     */
    ...
    /**
     * <p>
     * The lmder_f77 method minimizes the sum of the squares of m nonlinear functions in n variables by a modification
     * of the Levenberg-Marquardt algorithm. The user must provide a method which calculates the functions and the
     * Jacobian.
     * <p>
     * Translated by Steve Verrill on November 3, 2000 from the FORTRAN MINPACK source produced by Garbow, Hillstrom,
     * and More.<p>
     *
     * @param nlls   A class that implements the Lmder_fcn interface (see the definition in Lmder_fcn.java). See
     *               LmderTest_f77.java for an example of such a class. The class must define a method, fcn, that must have the form
     ...
     *               reductions in the sum of squares are at most ftol. Therefore, ftol measures the relative error desired in the sum
     *               of squares.
     * @param xtol   A nonnegative input variable. Termination occurs when the relative error between two consecutive
     *               iterates is at most xtol. Therefore, xtol measures the relative error desired in the approximate solution.
     * @param gtol   A nonnegative input variable. Termination occurs when the cosine of the angle between fvec and any
     *               column of the Jacobian is at most gtol in absolute value. Therefore, gtol measures the orthogonality desired
     *               between the function vector and the columns of the Jacobian.
     * @param maxfev A positive integer input variable. Termination occurs when the number of calls to fcn with iflag[1]
     *               = 1 has reached maxfev.
     * @param diag   An vector of length n. If mode = 1 (see below), diag is internally set. If mode = 2, diag must
     *               contain positive entries that serve as multiplicative scale factors for the variables.
     * @param mode   If mode = 1, the variables will be scaled internally. If mode = 2, the scaling is specified by the
     *               input diag. Other values of mode are equivalent to mode = 1.
     * @param factor A positive input variable used in determining the initial step bound. This bound is set to the
     *               product of factor and the euclidean norm of diag*x if nonzero, or else to factor itself. In most cases factor
     *               should lie in the interval (.1,100). 100 is a generally recommended value.
     * @param nprint An integer input variable that enables controlled printing of iterates if it is positive. In this
     *               case, fcn is called with iflag[1] = 0 at the beginning of the first iteration and every nprint iterations
     *               thereafter and immediately prior to return, with x, fvec, and fjac available for printing. fvec and fjac should
     *               not be altered. If nprint is not positive, no special calls of fcn with iflag[1] = 0 are made.
     * @param info   An integer output variable. If the user has terminated execution, info is set to the (negative) value
     *               of iflag[1]. See description of fcn. Otherwise, info is set as follows.
     *               <p>
     *               info = 0 improper input parameters.
     ...
     *               <p>
     *               info = 5 number of calls to fcn with iflag[1] = 1 has reached maxfev.
     *               <p>
     ...
     * @param nfev   An integer output variable set to the number of calls to fcn with iflag[1] = 1.
     * @param njev   An integer output variable set to the number of calls to fcn with iflag[1] = 2.
     * @param ipvt   An integer output array of length n. ipvt defines a permutation matrix P such that jac*P = QR, where
     *               jac is the final calculated Jacobian, Q is orthogonal (not stored), and R is upper triangular with diagonal
     *               elements of nonincreasing magnitude. column j of P is column ipvt[j] of the identity matrix.
    ...
    public static void lmder_f77(Lmder_fcn nlls, int m, int n, double[] x,
                                 double[] fvec, double[][] fjac, double ftol, double xtol, double gtol,
        ...
        // Check the input parameters for errors.
        if ((n <= 0) || (m < n) || (ftol < zero) || (xtol < zero)
    ...
    /**
     * <p>
     * The qrfac_f77 method uses Householder transformations with column pivoting (optional) to compute a QR
     * factorization of the m by n matrix A. That is, qrfac_f77 determines an orthogonal matrix Q, a permutation matrix
     * P, and an upper trapezoidal matrix R with diagonal elements of nonincreasing magnitude, such that AP = QR.
     * <p>
     * Translated by Steve Verrill on November 17, 2000 from the FORTRAN MINPACK source produced by Garbow, Hillstrom,
     * and More.<p>
     *
     ...
     * @param a      A is an m by n array. On input A contains the matrix for which the QR factorization is to be computed.
     *               On output the strict upper trapezoidal part of A contains the strict upper trapezoidal part of R, and the lower
     *               trapezoidal part of A contains a factored form of Q.
     * @param pivot  pivot is a logical input variable. If pivot is set true, then column pivoting is enforced. If pivot
     *               is set false, then no column pivoting is done.
     * @param ipvt   ipvt is an integer output array. ipvt defines the permutation matrix P such that A*P = Q*R. Column j
     ...
     * @param acnorm acnorm is an output array of length n which contains the norms of the corresponding columns of the
     *               input matrix A.
                ...
                if (n >= jp1) {
                    for (k = jp1; k <= n; k++) {
                        ...
                        if (pivot && (rdiag[k] != zero)) {
                            temp = a[j][k] / rdiag[k];
                            rdiag[k] *= Math.sqrt(Math.max(zero,
                                    one - (temp * temp)));
     ...
     * <pre>
     *    Ax = b ,     Dx = 0 ,
     * </pre> in the least squares sense.
     * <p>
     ...
     * columns, and R is an upper triangular matrix with diagonal elements of nonincreasing magnitude, then qrsolv_f77
     * expects the full upper triangle of R, the permutation matrix P, and the first n components of (Q transpose)b. The
     * system
     * <pre>
     ...
     *                 t     t
     *           Rz = Q b,  P DPz = 0 ,
     * </pre> where x = Pz. If this system does not have full rank, then a least squares solution is obtained. On output
     * qrsolv_f77 also provides an upper triangular matrix S such that
     * <pre>
     *            t  t              t
     *           P (A A + DD)P = S S .
     * </pre> S is computed within qrsolv_f77 and may be of separate interest.
     * <p>
     * Translated by Steve Verrill on November 17, 2000 from the FORTRAN MINPACK source produced by Garbow, Hillstrom,
     * and More.<p>
     *
     * @param n     The order of r.
     * @param r     r is an n by n array. On input the full upper triangle must contain the full upper triangle of the
     *              matrix R. On output the full upper triangle is unaltered, and the strict lower triangle contains the strict upper
     *              triangle (transposed) of the upper triangular matrix S.
     * @param ipvt  ipvt is an integer input array of length n which defines the permutation matrix P such that AP = QR.
     *              Column j of P is column ipvt[j] of the identity matrix.
     * @param diag  diag is an input array of length n which must contain the diagonal elements of the matrix D.
     * @param qtb   qtb is an input array of length n which must contain the first n elements of the vector (Q
     *              transpose)b.
     * @param x     x is an output array of length n which contains the least squares solution of the system Ax = b, Dx = 0.
     * @param sdiag sdiag is an output array of length n which contains the diagonal elements of the upper triangular
     *              matrix S.
     * @param wa    wa is a work array of length n.
     */
     ...
     * </pre> in the least squares sense, and dxnorm is the Euclidean norm of D*x, then either par is zero and
     * <pre>
     *           (dxnorm-delta) <= 0.1*delta ,
     * </pre> or par is positive and
     * <pre>
     *           abs(dxnorm-delta) <= 0.1*delta .
     * </pre> This method (lmpar_f77) completes the solution of the problem if it is provided with the necessary
     * information from the QR factorization, with column pivoting, of A. That is, if AP = QR, where P is a permutation
     * matrix, Q has orthogonal columns, and R is an upper triangular matrix with diagonal elements of nonincreasing
     * magnitude, then lmpar_f77 expects the full upper triangle of R, the permutation matrix P, and the first n
     * components of (Q transpose)b. On output lmpar_f77 also provides an upper triangular matrix S such that
     * <pre>
     *            t  t                t
     *           P (A A + par*DD)P = S S .
     * </pre> S is employed within lmpar_f77 and may be of separate interest.
     * <p>
     ...
     * @param qtb   qtb is an input array of length n which must contain the first n elements of the vector (Q
     *              transpose)b.
     * @param delta delta is a positive input variable which specifies an upper bound on the Euclidean norm of Dx.
     * @param par   par is a nonnegative variable. On input par contains an initial estimate of the Levenberg-Marquardt
     *              parameter. On output par contains the final estimate.
     * @param x     x is an output array of length n which contains the least squares solution of the system Ax = b,
     *              sqrt(par)*Dx = 0, for the output par.
     * @param sdiag sdiag is an output array of length n which contains the diagonal elements of the upper triangular
     *              matrix S.
    ...
    public static void lmpar_f77(int n, double[][] r, int[] ipvt,
                                 double[] diag, double[] qtb, double delta, double[] par, double[] x,
    ...
    /**
     * <p>
     * The lmdif1_f77 method minimizes the sum of the squares of m nonlinear functions in n variables by a modification
     * of the Levenberg-Marquardt algorithm. This is done by using the more general least-squares solver lmdif. The user
     * must provide a method that calculates the functions. The Jacobian is then calculated by a forward-difference
     * approximation.
     * <p>
     * Translated by Steve Verrill on November 24, 2000 from the FORTRAN MINPACK source produced by Garbow, Hillstrom,
     * and More.<p>
     *
     * @param nlls A class that implements the Lmdif_fcn interface (see the definition in Lmdif_fcn.java). See
     *             LmdifTest_f77.java for an example of such a class. The class must define a method, fcn, that must have the form
     *             <p>
     *             public static void fcn(int m, int n, double x[], double fvec[], int iflag[])
     *             <p>
     *             The value of iflag[1] should not be changed by fcn unless the user wants to terminate execution of lmdif_f77. In
     *             this case set iflag[1] to a negative integer.
     * @param m    A positive integer set to the number of functions [number of observations]
     * @param n    A positive integer set to the number of variables [number of parameters]. n must not exceed m.
     * @param x    On input, it contains the initial estimate of the solution vector [the least squares parameters]. On
    ...
    public static void lmdif1_f77(Lmdif_fcn nlls, int m, int n, double[] x,
        ...
        Minpack_f77.lmdif_f77(nlls, m, n, x, fvec, ftol, xtol, gtol, maxfev,
                epsfcn, diag, mode, factor, nprint, info, nfev, fjac, ipvt, qtf);
    ...
    /**
     * <p>
     * The lmdif_f77 method minimizes the sum of the squares of m nonlinear functions in n variables by a modification
     * of the Levenberg-Marquardt algorithm. The user must provide a method that calculates the functions. The Jacobian
     ...
     *               column of the Jacobian is at most gtol in absolute value. Therefore, gtol measures the orthogonality desired
     *               between the function vector and the columns of the Jacobian.
     * @param maxfev A positive integer input variable. Termination occurs when the number of calls to fcn is at least
     *               maxfev by the end of an iteration.
     * @param epsfcn An input variable used in determining a suitable step length for the forward-difference
     *               approximation. This approximation assumes that the relative errors in the functions are of the order of epsfcn.
     *               If epsfcn is less than the machine precision, it is assumed that the relative errors in the functions are of the
     ...
     * @param nprint An integer input variable that enables controlled printing of iterates if it is positive. In this
     *               case, fcn is called with iflag[1] = 0 at the beginning of the first iteration and every nprint iterations
     *               thereafter and immediately prior to return, with x and fvec available for printing. If nprint is not positive, no
     *               special calls of fcn with iflag[1] = 0 are made.
     * @param info   An integer output variable. If the user has terminated execution, info is set to the (negative) value
     *               of iflag[1]. See description of fcn. Otherwise, info is set as follows.
     *               <p>
     *               info = 0 improper input parameters.
     ...
     *               <p>
     *               info = 8 gtol is too small. fvec is orthogonal to the columns of the Jacobian to machine precision.
     * @param nfev   An integer output variable set to the number of calls to fcn.
     * @param fjac   An output m by n array. The upper n by n submatrix of fjac contains an upper triangular matrix R with
     ...
     *               P (jac *jac)P = R R,
     *               <p>
     *               where P is a permutation matrix and jac is the final calculated Jacobian. Column j of P is column ipvt[j] (see
     *               below) of the identity matrix. The lower trapezoidal part of fjac contains information generated during the
     *               computation of R.
     * @param ipvt   An integer output array of length n. ipvt defines a permutation matrix P such that jac*P = QR, where
     *               jac is the final calculated Jacobian, Q is orthogonal (not stored), and R is upper triangular with diagonal
     *               elements of nonincreasing magnitude. column j of P is column ipvt[j] of the identity matrix.
    ...
    /**
     * <p>
     * The fdjac2 method computes a forward-difference approximation to the m by n Jacobian matrix associated with a
     ...
     *               public static void fcn(int m, int n, double x[], double fvec[], int iflag[])
     *               <p>
     *               The value of iflag[1] should not be changed by fcn unless the user wants to terminate execution of fdjac2_f77. In
     *               this case iflag[1] should be set to a negative integer.
     * @param m      A positive integer set to the number of functions [number of observations]
     * @param n      A positive integer set to the number of variables [number of parameters]. n must not exceed m.
     * @param x      An input array.
     * @param fvec   An input array that contains the functions evaluated at x.
     * @param fjac   An output m by n array that contains the approximation to the Jacobian matrix evaluated at x.
     * @param iflag  An integer variable that can be used to terminate the execution of fdjac2. See the description of
     *               nlls.
     * @param epsfcn An input variable used in determining a suitable step length for the forward-difference
     *               approximation. This approximation assumes that the relative errors in the functions are of the order of epsfcn.
     *               If epsfcn is less than the machine precision, it is assumed that the relative errors in the functions are of the
    ...
    public static void fdjac2_f77(Lmdif_fcn nlls, int m, int n, double[] x,
                                  double[] fvec, double[][] fjac, int[] iflag, double epsfcn, double[] wa) {
        /*
