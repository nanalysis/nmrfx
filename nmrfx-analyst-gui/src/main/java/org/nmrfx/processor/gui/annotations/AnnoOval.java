...
public class AnnoOval extends AnnoShape {
    private static final Logger log = LoggerFactory.getLogger(AnnoOval.class);
    ...
    public AnnoOval(double x1, double y1, double x2, double y2,
    ...
    public void draw(GraphicsContextInterface gC, double[][] bounds, double[][] world) {
        try {
            ...
            if (stroke != null) {
                gC.setStroke(stroke);
                gC.strokeOval(xp1, yp1, width, height);
            }
            if (fill != null) {
                gC.setFill(fill);
                gC.fillOval(xp1, yp1, width, height);
            }
            if (isSelected()) {
                drawHandles(gC);
            }
