...
public class ScanTable {
    ...
    ListChangeListener<FileTableItem> filterItemListener = c -> {
        getGroups();
        getScannerLoader().ensureAllDatasetsAdded();
        selectionChanged();
    };
    ...
    private ScannerLoader getScannerLoader() {
    ...
    private void setDatasetVisibility(List<FileTableItem> showRows, Double curLvl) {
        if (activeDatasetAttributes.isEmpty()) {
            scannerLoader.ensureAllDatasetsAdded();
            setDatasetAttributes();
        }
        PolyChart chart = scannerTool.getChart();
        List<DatasetAttributes> datasetAttributesList = chart.getDatasetAttributes();
    ...
    private void mouseDragDropped(final DragEvent e) {
        ...
        if (db.hasFiles()) {
            ...
            if (file.isDirectory()) {
                Platform.runLater(() -> {
                    List<String> nmrFiles = NMRDataUtil.findNMRDirectories(file.getAbsolutePath());
                    initTable();
                    scannerLoader.loadScanFiles(file, nmrFiles);
                });
            } else {
                Platform.runLater(() -> scannerLoader.loadScanTable(file, this));
            }
        }
    ...
    public void openSelectedListFile() {
        ...
        File scanDir = scannerLoader.getScanDir();
        if (selItem >= 0) {
    ...
    void processColumns(File file, String[] headers, boolean[] notInteger, boolean[] notDouble, long firstDate, String firstDatasetName) {
                ...
                for (String columnType : columnTypes.keySet()) {
                    ...
                    if (VPAT.matcher(columnType).matches()) {
                        try {
                            ...
                            columnNum = Integer.parseInt(columnType.substring(1, colonPos));
    ...
    public void setChart() {
        ...
        if (currentChart != chart) {
            ...
            currentChart = chart;
            scannerLoader.loadFromDataset(this);
        }
        updateFilter();
    }
